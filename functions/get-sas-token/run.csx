...
public static async Task<HttpResponseMessage> Run(HttpRequestMessage req, TraceWriter log)
{
    ...
    log.Info("container=" + data.container.ToString());
    log.Info("blobNamer=" + data.blobName.ToString());
    log.Info("permissions=" + data.permissions.ToString());


    var storageAccount = CloudStorageAccount.Parse(ConfigurationManager.AppSettings["SourceStorageConnection"]);
...
private static SharedAccessBlobPolicy CreateAdHocSasPolicy(SharedAccessBlobPermissions permissions)
{
    ...
    return new SharedAccessBlobPolicy() {
        ...
        SharedAccessExpiryTime = DateTime.UtcNow.AddHours(Convert.	ToInt32(ConfigurationManager.AppSettings["SasTokenTTLHours"])),
